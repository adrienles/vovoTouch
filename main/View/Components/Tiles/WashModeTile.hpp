...
#include "../Buttons/BidetButton.hpp"

LV_FONT_DECLARE(sf_comp_round_semibold_24);

template <bool hasTurboMode>
class WashModeTile : public Tile
{
public:
    explicit WashModeTile(lv_obj_t* parent, const lv_image_dsc_t *iconSrc, const char *title) : Tile(parent) {
        setupStyle();

        createDescriptionContainer(iconSrc, title);
        createButtonContainer();
    }

    const std::array<BidetButton*, hasTurboMode ? 3 : 2>& getButtons() {
        return buttons;
    }
...
private:
    ...
    void createDescriptionContainer(const lv_image_dsc_t *iconSrc, const char *title)
    {
    ...
    std::array<BidetButton*, hasTurboMode ? 3 : 2> buttons;
};